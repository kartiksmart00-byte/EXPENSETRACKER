...
def income():
    ...
    conn = get_connection()
    cursor = conn.cursor(dictionary=True)
    cursor.execute("SELECT source, amount, date FROM INCOME WHERE user_id=%s ORDER BY date DESC LIMIT 5",
                   (session['user_id'],))
...
def expense():
    ...
    conn = get_connection()
    cursor = conn.cursor(dictionary=True)
    cursor.execute("SELECT category, amount, date FROM EXPENSE WHERE user_id=%s ORDER BY date DESC LIMIT 5",
                   (session['user_id'],))
